...
export default function AddStudentForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.schoolId) {
      error("School is required!", { duration: 3000, position: "top-right", description: "Please search and select a school first." })
      return
    }

    if (!formData.name) {
      error("Name is required!", { duration: 3000, position: "top-right", description: "Please enter a name." })
      return
    }
    if (!formData.class) {
      error("Class is required!", { duration: 3000, position: "top-right", description: "Please enter a class." })
      return
    }
    if (!formData.gender) {
      error("Gender is required!", { duration: 3000, position: "top-right", description: "Please select a gender." })
      return
    }
    if (!formData.parentName) {
      error("Parent Name is required!", { duration: 3000, position: "top-right", description: "Please enter a parent name." })
      return
    }
    if (!formData.parentContact) {
      error("Parent Contact is required!", { duration: 3000, position: "top-right", description: "Please enter a parent contact." })
      return
    }
